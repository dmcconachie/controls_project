...
public:
    ...
    Mesh(const std::vector<cv::Point2f>& vec, int rows, int cols);
    ...
    std::vector<cv::Point2f> getVectorFromMesh();
	/** \brief  Builds mesh from vector of 2d points with the provided number of rows and cols. This is not interpolated.*/
    void makeMeshFromVector(const std::vector<cv::Point2f>& vec, int cols, int rows);
    ...
    static cv::Rect getVectorBounds(std::vector<cv::Point>& data);
    ...
    std::vector<cv::Vec2i> controlIndex; //Each point Vec2i in this vector stores the row and col of a control point in the mesh.
};
